...
# Ensure x-axis ranges match the lengths of the smoothed data
...
I made the graph viewable by using **`axs`**, which allows a single figure to have multiple subplots. I used the `fig, axs = plt.subplots(2, 1, figsize=(15, 10), dpi=100)` to provide two vertically stacked subplots where they share the same x-axis, with **`axs[0]`** containing the humidity data and **`axs[1]`** containing the temperature. To reduce clutter and improve readability, I applied downsampling by plotting every 100th data point and used a moving average with a window size of 50 to smooth out noise in both datasets. This highlighted trends more clearly while preserving overall patterns. Figure size and DPI were increased for higher resolution, so that details are easily discernerable. Each subplot was customized with different colors, markers, thicker lines for the smoothed data, grid lines, bold titles, and clear axis labels. The addition of **`plt.tight_layout()`** ensured that the spacing between the subplots was optimal, not overlapping, and thus more readable. These changes together make the visualization clear, uncluttered, and easy to interpret.
